...
const ForgotPass = ({ setForgotpass }) => {
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      ...
      alert("Password reset link sent to your email! Please Check message in your mail at Spam");
      setForgotpass(false); // Redirect back to sign in
    } catch (error) {
      alert(error.message);
    }
  };
  ...
  return (
    <div className='absolute right-0 left-0 top-0 bottom-0 px-3.5'>
      <form  
        ...
        className="max-w-[30rem] p-10 bg-black my-50 mx-auto text-white rounded-lg">
        <h1 className="py-4 text-4xl font-bold">Reset Password</h1>
        <input 
          type="email"
          placeholder="Enter your email"
          value={email}
